...
lookup vm addr = if addr < 32768
                 ...
                 else case addr of 32768 -> view (registers . r0) vm
...
ipush vm instr = let a' = view a instr
...
ieq vm instr = let addr = view a instr
                   result = if view b instr == view c instr then 1 else 0
...
ijmp vm instr = let addr = view a instr
                    vm' = setPC vm addr -- jump
                in return vm'

ijt :: Vm -> Instruction -> IO Vm
ijt vm instr = let cond = view a instr
                   addr = view b instr
               in if cond /= 0
...
ijf vm instr = let cond = view a instr
                   addr = view b instr
               in if cond == 0
                  then return (setPC vm addr) -- jump
                  else return (incPC vm 3)    -- no jump
...
iadd vm instr = let addr = view a instr
                    result = view b instr + view c instr -- add
...
imult vm instr = let addr = view a instr
                     result = view b instr * view a instr -- multiply
                     result' = mod result 32768           -- modulo
                     vm' = store vm addr result'          -- update memory
                     vm'' = incPC vm' 4                   -- increment pc
                 in return vm''
...
imod vm instr = let addr = view a instr
                    result = view b instr `mod` view c instr -- modulo
                    vm' = store vm addr result               -- update memory
                    vm'' = incPC vm' 4                       -- increment pc
                in return vm''
...
iand vm instr = let addr = view a instr
                    result = view b instr .&. view c instr -- bitwise and
                    vm' = store vm addr result             -- update memory
                    vm'' = incPC vm' 4                     -- increment pc
                in return vm''
...
ior vm instr = let addr = view a instr
                   result = view b instr .|. view c instr -- bitwise or
                   vm' = store vm addr result             -- update memory
                   vm'' = incPC vm' 4                     -- increment pc
               in return vm''
...
inot vm instr = let addr = view a instr
                    result = complement (view b instr) -- complement (not sure if it's a 15-bit complement though)
                    vm' = store vm addr result         -- update memory
                    vm'' = incPC vm' 3                 -- increment pc
                in return vm''
...
iwmem vm instr = let addr = view a instr     -- get destination address
                     val = view b instr      -- get value to be written
...
icall vm instr = let pc = view (registers . progCtr) vm -- get current pc
                     ...
                     dest = view a instr                -- get destination addr
...
iout vm instr = do putChar $ chr (view a instr) -- output char
